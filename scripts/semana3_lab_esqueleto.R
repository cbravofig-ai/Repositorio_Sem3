...
fecha_censo <- as.Date("2024-01-01")        # Date: usa el formato "AAAA-MM-DD"
...
anios <- seq(2018, 2023, by = 1)
# anios <- 2018:2023                    (forma más sencilla de hacerlo)
...
salario[-length(salario)]           # todos MENOS el sexto (pista: posición negativa)
...
sum(salario > 500000)        # ¿cuántos?
mean(salario > 500000)        # ¿qué proporción?
...
# RESPUESTA: Puede ser porque el último valor del vector (1.250.000) es un dato 
# bastante mayor, lo que altera y aumenta matemáticamente el promedio.
#
...
# x <- c(50000, 120000, 80000, 250000, 100000)
...
# ¿Funcionó a la primera? Sí
# ¿Usó la misma forma que aprendiste en el PASO 4, u otra distinta? Utilizó la 
# misma que aprendimos en clases.
# ¿Entiendes cada parte de lo que te dio? Si no, ¿qué parte no? Lo entiendo.
#
...
#
# MI PREGUNTA: ¿Cuántas personas con nivel de educación media superan la 
# mediana del salario?

# Para obtener los salarios de quienes tienen educación media
salario [nivel == "media"]

# Para saber si hay datos con educación media que superan la media
salario [nivel == "media"] > median(salario)

# Para sumar cuántas personas con educación media tienen un salario por sobre 
# la mediana.
sum(salario[nivel == "media"] > median(salario))

# Sólo 1 persona con nivel de educación media, supera la mediana del salario.

# -----------------------------------------------------------------------------
# AUTOEVALUACIÓN antes de entregar
# -----------------------------------------------------------------------------
# [ Sí ] No quedan ____ sin completar.
# [ Sí ] El script corre completo de una vez (reinicia R y córrelo entero).
# [ Sí ] Verifiqué los tipos antes de calcular.
# [ Sí ] Respondí las dos preguntas guía en los comentarios.
# [ Sí ] Documenté el resultado de la consulta a ChatGPT.
