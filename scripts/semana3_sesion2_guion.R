...
#
# Nombre: Camila Bravo Figueroa      Fecha: 25-08-2026
#
...
seq(10000, 100000, by = 1000) # Para crear secuencias
seq(0, 1, by = 0.1)

rep("Ñuble", 3)             # repetir un valor

anio <- rep(seq(2007, 2009, by = 1), 2)
region <- c(rep("Ñuble",3), rep("Maule", 3))
...
length(ingresos)              # 5 elementos
ingresos[6]                   # NA, no existe el elemento 6 en el vector ingresos

ingresos[c(1, 3, 5)]           # primero, tercero y quinto
ingresos[-2]                   # TODOS menos el segundo (índice negativo)
ingresos[-length(ingresos)]
# ingresos <- ingresos[-length(ingresos)] # Eliminar el último objeto de un vector

# Eliminar objetos de un vector sólo cuando haya seguridad de no estar erróneo, 
# porque esto implicará que ese dato ya no estará más en la memoria. Una opción
# es crear un vector tipo "ingresos2" pero esto implica guardar un segundo vector,
# que cuando utiliza datos pesados, sale afecta la memoria.
...
# ingresos_sobre_400mil <- ingresos[ingresos > 400000] ; filtrar datos asignando 
# nuevos vectores a la data limpia para utilizar.

# 💡 Dentro del [ ] va un vector lógico (S3S1). Es la base de filter() (S5).

## Contar y resumir con condiciones -------------------------------------------
# 🔮 PREDICE: sum(...) ->     mean(...) ->
sum(ingresos > 400000)         # CUÁNTOS superan 400.000        -> 3


mean(ingresos > 400000)        # qué PROPORCIÓN los supera      -> 0,6 
mean(ingresos[ingresos > 400000])   # promedio SOLO de los que superan  -> 626666.7
mean(ingresos)                  # promedio de los ingresos
