...
#   2000 + 500   -> 2500
#   "Ñuble" + 1  -> No se puede, el programa indica "argumento no numérico para 
# operador binario.
#   TRUE + TRUE  -> (1+1=2) 
...
tasa_crecimiento <- 0.32
12000 * ( 1 + tasa_crecimiento )
# [1] 15840
...
# Para comparar en igualdad, deben utilizarse dos signos igual, de lo contrario 
# asignará el valor de una a la otra como "esto es igual a esto".
ingreso == linea_pobreza      # [1] FALSE FALSE FALSE FALSE (son iguales)
ingreso != linea_pobreza      # [1] TRUE TRUE TRUE TRUE     (son distintos)

# 🔮 PREDICE: ¿cuánto darán estas dos líneas?  sum(...) ->     mean(...) ->
sum(ingreso < linea_pobreza)   # ¿cuántos bajo la línea? (2)
mean(ingreso < linea_pobreza)  # ¿qué proporción? (= tasa de pobreza) (0,5)
...
NSE <- factor(c("medio", "alto", "bajo", "alto"), 
...
# o  también as.Date("2026-12-31") - as.Date("2026-08-25")
...
factor_estado_civil <- factor(estado_civil,
                              level= c("soltero", "casado", "viudo"))

as.factor(estado_civil)   # Misma función que el código anterior.
class(as.factor(estado_civil))  # [1] "factor"
...
c(1, 2, "tres")                  # -> character, porque tres es texto.
class(c(1, 2, "tres"))

c(TRUE, 5, 10)                   # -> numeric, porque TRUE es igual a 1.
class(c(TRUE, 5, 10))
...
class(educ)     # ordered factor
class(fecha)    # date

is.character(region)    # TRUE
is.factor(educ)         # TRUE
is.numeric(fecha)       # FALSE
